...
class EventHub:
    def __init__(self, max_clients=8, queue_depth=16):
        ...
        # Keyed by client object identity so removal never matches by contents.
        self._clients = {}
    ...
    async def wait(self, client):
        while True:
            entry = self._clients.get(id(client))
            if entry is None:
...
def create_app(state, hub=None):
    ...
    async def stream(request):
        ...
        async def sender():
            while not ws.closed:
                try:
                    event = await asyncio.wait_for(hub.wait(client_id), timeout=30)
